...
namespace Moonvalk.Animation {
	...
	public abstract class BaseMoonTweenHandler<Unit, TweenType, ParentType> : IMoonTweenHandler
		...
		/// </summary>
		public MoonTweenProperty Property { get; private set; }
		...
		/// </summary>
		public TweenType Tween { get; private set; }
		...
		protected void bindData(ParentType object_, MoonTweenProperty property_) {
			this._value = this.getInitialPropertyValue(object_, property_);
		...
		/// <param name="onComplete_">An action to be run when this Tween is complete. This is primarily used
		/// to remove a Tween reference once finished.</param>
		protected void setupTween(Unit target_, MoonTweenParams parameters_, bool start_, Action onComplete_) {
			this.Tween = new TweenType() { StartOnTargetAssigned = start_ };
			this.Tween.SetReferences(() => ref this._value).SetParameters(parameters_);
		...
		protected abstract Unit getInitialPropertyValue(ParentType object_, MoonTweenProperty property_);
	}
}
