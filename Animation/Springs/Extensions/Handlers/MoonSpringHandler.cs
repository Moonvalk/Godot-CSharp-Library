...
namespace Moonvalk.Animation {
	...
	public class MoonSpringHandler<ParentType> : BaseMoonSpringHandler<float, MoonSpring, ParentType> {
				...
				case Control control:
					switch (property_) {
						case MoonSpringProperty.Rotation:
							return () => { control.RectRotation = this._value; };
					}
					break;
				case Node2D node:
					switch (property_) {
						case MoonSpringProperty.Rotation:
							return () => { node.RotationDegrees = this._value; };
					}
					break;
			}
			return null;
		...
		protected override float getInitialPropertyValue(ref ParentType object_, MoonSpringProperty property_) {
			switch (object_) {
				case Control control:
					switch (property_) {
						case MoonSpringProperty.Rotation:
							return control.RectRotation;
					}
					break;
				case Node2D node:
					switch (property_) {
